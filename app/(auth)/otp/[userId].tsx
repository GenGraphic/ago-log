...
const Otp = () => {
    ...
    const handleValidate = useCallback(async(otpCode: string) => {
        if (!otpCode || otpCode.length !== 6 || !userId) {
            console.log("Validation blocked - userId:", userId, "otpCode length:", otpCode.length);
            return;
        }
        ...
        try {
            ...
            if(!result.success) {
                ...
                Toast.show({ type: 'error', text1: 'Invalid OTP', text2: result.message });
                setOtp("");
                return;
            };
    ...
    const resendOtp = async() => {
        try {
            ...
            Toast.show({ type: 'success', text1: 'Email sent!', text2: 'We have sent you the email one more time. Check Spam too.' })
            setOtp("");
        }finally {
            setLoading(false)
        }
                ...
                <View style={globalStyles.mainContainer}>
                    {loading ? (
                        ...
                        <View style={styles.form}>
                            <ThemedText type='title' style={styles.title}>Enter OTP Code</ThemedText>
                            <ThemedText style={styles.subtitle}>Check your email for the verification code</ThemedText>
                            ...
                            <ThemedText style={styles.helpText}>We have sent you an email with your one time password.</ThemedText>

                            <TouchableOpacity style={{ alignSelf: 'center' }} onPress={resendOtp} disabled={loading}>
                                <ThemedText type='link'>Resend Code</ThemedText>
                            </TouchableOpacity>
                            
                            <ThemedText style={styles.debugText}>Code sent to: {userEmail}</ThemedText>
                        </View>
                    )}
                </View>
            </SafeAreaView>
