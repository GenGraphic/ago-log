...
function statusColor(entry: Entry): string {
  switch (entry.status) {
    case EntryStatus.EXPIRED:  return StatusColors.expired;
    case EntryStatus.ARCHIVED: return StatusColors.archived;
    default:                   return StatusColors.active;
  }
}
...
function InfoRow({ label, value, sensitive = false }: InfoRowProps) {
  ...
  return (
    <View style={detailStyles.infoRow}>
      <ThemedText style={[detailStyles.infoLabel, { color: `${icon}80` }]}>{label}</ThemedText>
      <View style={detailStyles.infoValueRow}>
        ...
        {sensitive && (
          <TouchableOpacity onPress={() => setRevealed(r => !r)} style={{ marginLeft: 8 }}>
            <Feather name={revealed ? 'eye-off' : 'eye'} size={14} color={`${icon}80`} />
          </TouchableOpacity>
        )}
      </View>
    </View>
...
export default function DocumentDetailsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (res.success) {
      setEntry(res.data);
      if (res.data.imageId) {
        const imgRes = await getImagePreview(ENTRIES_IMAGES_BUCKET_ID, res.data.imageId);
  ...
  if (loading) {
    return (
      <SafeAreaView style={[detailStyles.fill, { backgroundColor: '#0B1120', alignItems: 'center', justifyContent: 'center' }]}>
  ...
  return (
    <SafeAreaView style={[detailStyles.fill, { backgroundColor: '#0B1120' }]}>
      <View style={detailStyles.topBar}>
        <TouchableOpacity style={detailStyles.backBtn} onPress={() => router.back()}>
          <Feather name="arrow-left" size={20} color={icon} />
        </TouchableOpacity>
        <View style={detailStyles.logoRow}>
          <ThemedText style={[detailStyles.logoText, { color: text }]}>AGO_LOG</ThemedText>
          <View style={[detailStyles.logoDot, { backgroundColor: tint }]} />
        </View>
        <TouchableOpacity style={detailStyles.backBtn} onPress={() => router.push(`/(main)/edit-entry/${id}`)}>
      ...
      <ScrollView style={detailStyles.scroll} contentContainerStyle={detailStyles.scrollContent} showsVerticalScrollIndicator={false}>
        {imageUri && (
          <Image source={{ uri: imageUri }} style={detailStyles.image} contentFit="cover" />
        )}
        <ThemedText style={[detailStyles.typeLabel, { color: `${icon}80` }]}>
          {entry.entryType.replace(/_/g, ' ').toUpperCase()}
        </ThemedText>
        <ThemedText style={[detailStyles.title, { color: text }]}>{entry.title}</ThemedText>
        <View style={[detailStyles.statusPill, { borderColor: color }]}>
          <View style={[detailStyles.statusDot, { backgroundColor: color }]} />
          <ThemedText style={[detailStyles.statusText, { color }]}>{entry.status.toUpperCase()}</ThemedText>
        </View>
        {days !== null && (
          <View style={[detailStyles.countdownCard, { borderLeftColor: color }]}>
            <ThemedText style={[detailStyles.countdownNumber, { color }]}>
              {days < 0 ? Math.abs(days) : days}
            </ThemedText>
            <ThemedText style={[detailStyles.countdownLabel, { color: `${icon}99` }]}>
        ...
        <View style={[detailStyles.section, { borderColor: `${tint}18` }]}>
          <ThemedText style={[detailStyles.sectionTitle, { color: `${icon}80` }]}>DETAILS</ThemedText>
        ...
        {entry.notes && (
          <View style={[detailStyles.section, { borderColor: `${tint}18` }]}>
            <ThemedText style={[detailStyles.sectionTitle, { color: `${icon}80` }]}>NOTES</ThemedText>
            <ThemedText style={[detailStyles.notes, { color: `${icon}CC` }]}>{entry.notes}</ThemedText>
          </View>
        )}
      ...
      <View style={detailStyles.footer}>
        <TouchableOpacity style={[detailStyles.editBtn, { borderColor: tint }]} onPress={() => router.push(`/(main)/edit-entry/${id}`)}>
          <Feather name="edit-2" size={16} color={tint} />
          <ThemedText style={[detailStyles.editBtnText, { color: tint }]}>EDIT</ThemedText>
        </TouchableOpacity>
...
const detailStyles = StyleSheet.create({
  ...
  typeLabel:       { fontSize: 10, fontWeight: '700', letterSpacing: 2, marginBottom: 4 },
  title:           { fontSize: 26, fontWeight: 'bold', letterSpacing: 0.5, marginBottom: 12 },
  statusPill:      { flexDirection: 'row', alignItems: 'center', gap: 6, alignSelf: 'flex-start', borderWidth: 1, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, marginBottom: 20 },
  statusDot:       { width: 6, height: 6, borderRadius: 3 },
  statusText:      { fontSize: 10, fontWeight: '700', letterSpacing: 1.5 },
  countdownCard:   { borderLeftWidth: 3, paddingLeft: 16, paddingVertical: 12, marginBottom: 24, gap: 2 },
  ...
  infoRow:         { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#1E2A3A' },
  ...
  editBtn:         { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 10, borderWidth: 1 },
  editBtnText:     { fontSize: 12, fontWeight: '800', letterSpacing: 2 },
  deleteBtn:       { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 10, backgroundColor: '#FF3B30' },
  deleteBtnText:   { fontSize: 12, fontWeight: '800', letterSpacing: 2, color: '#fff' },
});